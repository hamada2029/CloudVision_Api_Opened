...
class FullText:
    ...
    def __init__(self, jp, hp, line_tolerance=5):
        self.jp = jp
        self.hp = hp
        self.j = json.loads(jp.read_text(), object_hook=D)
        if 'responses' not in self.j:
            if 'fullTextAnnotation' not in self.j:
                self.pages = []
            else:
    ...
    def to_hocr(self):
        if len(self.hocrpages) > 1:
            raise ValueError('multiple hocr pages')

        hocrpage = self.hocrpages[0]
        s = template(
            'page.html',
